...
// McpSearchRequest MCP搜索请求参数简化，避免AI乱使用参数
type McpSearchRequest struct {
	Keyword      string        `json:"kw" jsonschema:"搜索关键词"`
	CloudTypes   []string      `json:"cloud_types" jsonschema:"指定返回的网盘类型列表，不指定则返回所有类型，支持的类型：baidu,aliyun,quark,tianyi,uc,mobile,115,pikpak,xunlei,123,magnet,ed2k,others"`
	ForceRefresh bool          `json:"refresh" jsonschema:"强制刷新，不使用缓存"`
	Filter       *FilterConfig `json:"filter,omitempty" jsonschema:"过滤配置，用于过滤返回结果"`
}

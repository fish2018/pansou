...
**真正的硬件盲点（调度器侧已修，应用侧待决策）**：`GOMAXPROCS` 以及几处"核数 × N"的自动推导
...
而实际只有 1 核配额——放大数倍并触发限流。应用侧仍是宿主机核数：`runtime/debug.go` 里 `NumCPU()` 返回 `numCPUStartup`
（注释：可用 CPU 集合在**进程启动时**向操作系统查询一次），它不读 cgroup，因此
`ASYNC_MAX_BACKGROUND_WORKERS`（核数 × 5）、`HTTP_MAX_CONNS`（核数 × 200）在 `cpus: 1` 的容器里
仍按宿主核数计算。修法是把这几处改用 `runtime.GOMAXPROCS(0)`（Go 1.25 起即配额感知，
非容器环境取值与 `NumCPU()` 完全相同，属可证明的 no-op）；
是否要动取决于目标部署是否真的用 `cpus:` 限定 CPU。

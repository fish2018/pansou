...
初值 = 任务数 / 4（下限 8，并按 CPU 核数设上界，见下）；地板随观测到的 p90 以 1/8 比例松弛；
上限为出口上限（`OUTBOUND_MAX_CONCURRENCY`，默认 128）。
...
**并发不由硬件规格推导，而由观测推导**——这是刻意的：规格只能猜出机器的能力，观测能测出来。
唯一"拍"出来的数是起始值，现在也按 CPU 核数设了上界（每核 4 个，下限 8 兜底）。

| 环节 | 并发怎么定 | 是否看硬件 |
| --- | --- | --- |
| 插件扇出 | `min(max(请求 conc, 任务数), 自适应限值)` | ✗ 不看硬件 |
| 自适应控制器 | 观测排队 / 出口门丢弃 → ×0.85；顺畅 → +12.5% | 间接（测行为，不猜规格） |
| 起始值 | 任务数/4，按 CPU 核数（×4）设上界，下限 8 | ✓ |
| 后台工作者 | CPU 核数 × 5 | ✓ |
| HTTP 最大连接数 | CPU 核数 × 200 | ✓ |
| 缓存分片 | 内存缓存按核数、磁盘缓存按核数 × 2 | ✓ |

**核数上界在常见机器上是 no-op**：8 核机器上 71 个任务的起始值仍是 71/4 = 17，与加此上界之前
逐位一致（既有 6 个自适应用例无一改动即为证据）；只有在核数很少时才会把起始值压下来
（2 核 → 8，1 核 → 下限 8 兜底）。

**为什么只看 CPU 不看内存**：单个并发任务的内存占用已被每个响应的读取上限兜住
（`util.ReadAllLimited`），不随并发线性膨胀；而"系统可用内存"在 Go 里要么拿不到
（runtime 只暴露堆），要么需要平台相关代码或第三方依赖，收益不抵复杂度。
真正的安全网是"排队或丢弃一出现就把并发降 15%"。

**已知不对称（未改动，留待评估）**：TG 通道路径的并行度取频道数（现为 111），既不经过出口门
也不参与自适应。实测该路径工作正常（可达时 111 个频道一轮返回 459 条），且 111 < 出口上限 128，
即使接入出口门也不会生效；若将来频道数超过出口上限，需要重新评估是否给它也加自适应。

...
**真正的硬件盲点（未改动，待决策）**：`GOMAXPROCS` 以及几处"核数 × N"的自动推导都读**宿主机**核数，
在容器里读不到 `--cpus` / cgroup 配额。本项目 Go 1.24（`Dockerfile` 为 `golang:1.24-alpine`），
而 Go 到 1.25 才让 `GOMAXPROCS` 感知 cgroup 配额。Uber automaxprocs 实测（容器配额 2 核）：
...
而实际只有 1 核配额——放大数倍并触发限流。修法是这几个数改读 cgroup 配额
（自研约 30 行读 `cpu.max` / `cpu.cfs_quota_us`，或引入 `go.uber.org/automaxprocs` 一行 import）。
属部署形态相关改动，先确认目标部署是否真的限定 CPU 再动。
